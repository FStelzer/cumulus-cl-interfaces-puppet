...
Puppet::Type.type(:cumulus_bond).provide :cumulus do
  ...
  def build_desired_config
    ...
    %w(vids pvid access portmcrouter).each do |attr|
    ...
    %w(link_down mstpctl_portnetwork mstpctl_bpduguard mstpctl_portbpdufilter mstpctl_portadminedge clag_id mtu gateway vrf).each do |attr|
